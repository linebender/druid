...
use crate::{
    ...
    PaintCtx, RawMods, SysMods, TimerToken, UpdateCtx, Widget, Data
};
...
/// A widget that allows user text input and editing.
#[derive(Debug, Clone)]
...
impl<E: 'static + EditableText + Data + std::string::ToString> Widget<E> for TextBox<E> {
    ...
    fn update(&mut self, ctx: &mut UpdateCtx, _old_data: Option<&E>, _data: &E, _env: &Env) {
        ctx.invalidate();
    }
