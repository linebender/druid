...
impl<T: Data> Widget<T> for Either<T> {
    #[instrument(skip(self, ctx, event, data, env), fields(branch = self.current))]
    ...
    #[instrument(skip(self, ctx, _old_data, data, env), fields(branch = self.current))]
    ...
    #[instrument(skip(self, ctx, bc, data, env), fields(branch = self.current))]
    ...
    #[instrument(skip(self, ctx, data, env), fields(branch = self.current))]
