...
fn simple_disable() {
    ...
    let check_states = |desired: [Option<bool>; 4]| {
        assert_eq!(desired[0], disabled_0.get());
        assert_eq!(desired[1], disabled_1.get());
        assert_eq!(desired[2], disabled_2.get());
        assert_eq!(desired[3], disabled_3.get());
    };
    ...
    Harness::create_simple((), root, |harness| {
        ...
        assert_eq!(harness.window().focus_chain().len(), 2);
        //This is intended the widget should not receive an event!
        harness.submit_command(Command::new(CHANGE_DISABLED, true, id_1));
        check_states([Some(true), None, Some(false), Some(true)]);
        assert_eq!(harness.window().focus_chain().len(), 2);
    })
}
...
fn disable_tree() {
    ...
    let check_states = |desired: [Option<bool>; 6]| {
        assert_eq!(desired[0], disabled_0.get());
        assert_eq!(desired[1], disabled_1.get());
        assert_eq!(desired[2], disabled_2.get());
        assert_eq!(desired[3], disabled_3.get());
        assert_eq!(desired[4], disabled_4.get());
        assert_eq!(desired[5], disabled_5.get());
    };
    ...
    Harness::create_simple((), root, |harness| {
        harness.send_initial_events();
        check_states([None, None, None, None, None, None]);
